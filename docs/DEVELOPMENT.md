...
两种摘要均保留身份、警告与关联信息，并排除外部链接。技术资料单独复制，
仅取 `ds` / `ref` 类链接，移除 UTM 参数并保留功能参数。
...
- PN/FID 共用 `DecodeResultPanel.vue` 与分组规格表。结果区约 800px 居中，宽屏每行最多
  四组短参数，手机保留两组短参数，长值自然换行。标量字段完整展示；控制器沿用可展开、
  ...
  型号旁的芯片类型属于身份信息，省略 Type/类型前缀并使用次要文字色；正文参数仍保留标签。
  中文标签使用统一列宽对齐数值；头部自然换行，空间足够时身份和复制按钮保持同一行。
  复制只提供整个分组、整个结果的简略/完整摘要和技术资料分组，不提供单字段或输入摘要按钮。
  常规 PN → ID、FID → PN 关联直接显示紧凑链接；特殊关系和搜索动作保留说明，关联附加字段不省略。
...
- 查询建议共用 `QuerySuggestionInput.vue`；涉及共享组件时检查 PN 与 Flash ID 两处。
  ...
  移动端导航不显示背景遮罩，点选页面后立即收起；桌面导航保持原有常驻/收起行为。
