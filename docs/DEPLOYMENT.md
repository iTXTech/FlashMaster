...
入口保持可见但禁用，不展示操作说明，也不主动推广。取消或调用失败后须等待浏览器再次提供安装事件。
...
PN/Flash ID 成功解码后延迟约 1.5 秒，在结果下方显示提示；输入框仍有焦点、键盘展开、
弹窗/临时侧栏打开或页面隐藏时延后显示。每个标签页会话最多出现一次，刷新也不会重复。
“稍后”、打开添加引导或响应系统安装确认后，30 天内不再主动提示。设置中的“显示安装提示”
只控制主动提示，侧栏仍可手动安装。开关及冷却时间由 `src/store/index.js` 存在本机，
不跟随版本更新重置，也不发送安装统计。
